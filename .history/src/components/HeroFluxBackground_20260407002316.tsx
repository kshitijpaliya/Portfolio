    ...
    const draw = (time: number) => {
      ...
      for (let i = 0; i < lineCount; i++) {
        ...
        for (let p = 0; p <= points; p++) {
          ...
          const taper = 0.2 + Math.pow(centerFade, 1.4) * 0.9;
        ...
        const flareWidth = 0.6 + Math.pow(Math.abs((0.5 - 0.5)), 1.2);
        ctx.lineWidth = (0.7 + fade * 0.9) * 0.75;
        ctx.stroke();

        const glowAlpha = 0.025 + fade * 0.045;
        ctx.strokeStyle = `rgba(99, 102, 241, ${glowAlpha})`;
        ctx.lineWidth = (2.2 + fade * 3.0) * 0.8;
        ctx.stroke();
