    ...
    const draw = (time: number) => {
      ...
      for (let i = 0; i < lineCount; i++) {
        ...
        for (let p = 0; p <= points; p++) {
          ...
          const edge = Math.pow(centerFade, 1.35);
          const taper = 0.2 + edge * 0.9;
          const wave1 = Math.sin(x * 0.008 + t * speed + phase);
          const wave2 = Math.sin(x * 0.02 + t * (speed * 1.3) + phase * 1.7);
          const y = baseY + (wave1 * 0.7 + wave2 * 0.3) * amplitude * (0.4 + taper * 0.75);

          linePoints.push({ x, y, edge });
        }
        ...
        for (let p = 1; p < linePoints.length; p++) {
          ...
          const widthScale = 0.32 + edge * 0.98;

          ctx.beginPath();
          ctx.moveTo(prev.x, prev.y);
          ctx.lineTo(curr.x, curr.y);
          ctx.strokeStyle = `rgba(129, 140, 248, ${alpha * (0.45 + edge * 0.55)})`;
          ...
          ctx.moveTo(prev.x, prev.y);
          ctx.lineTo(curr.x, curr.y);
          ctx.strokeStyle = `rgba(99, 102, 241, ${glowAlpha * (0.35 + edge * 0.65)})`;
