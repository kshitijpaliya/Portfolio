...
export default function HeroFluxBackground() {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      for (let i = 0; i < lineCount; i++) {
        ...
        const baseY = bandCenter + (progress - 0.5) * bandRange + wobble - height * 0.06 + edgeLift * height * 0.07;
        ...
        for (let p = 0; p <= points; p++) {
          ...
          const y = baseY + (wave1 * 0.7 + wave2 * 0.3) * amplitude * (0.35 + taper * 1.1);
  ...
  return (
    <canvas
      ref={canvasRef}
      className="absolute inset-0 h-full w-full -translate-y-6 pointer-events-none z-0"
      aria-hidden="true"
    />
  );
}
